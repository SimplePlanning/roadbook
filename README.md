...
3. **调整**：左侧列表可改名、上移/下移、删除；地图上标记可拖动微调；相邻点之间自动画驾车路线（分段缓存，失败的段显示虚线并给出提示条，点提示条可重试）。每个点下方显示距上一点的直线里程，列表上方显示全程直线总里程
4. **存到路书库**：点「存到路书库」，选一个文件夹（或新建）保存——比如建个「新疆自驾」文件夹，里面放「第一天」「第二天」两份路书（路书名称在此时填写；编辑面板的名称框只读，改名在路书库里进行）。点面板右上「路书库」可打开 / 改名 / 删除，每份路书显示地点数和直线总里程；点「轨迹」可把单份路书或整个文件夹的轨迹叠加显示在地图上（再点一次或点地图左上角图例的 ✕ 隐藏，不影响当前编辑）；数据存在浏览器 localStorage
