...
6. **手机导航**：打开路书后，顶部绿色按钮「导航全部」——3 个点以内直接调起高德 App（官方 URI 只支持 1 个途经点）；更多点在**手机上无法一次导入**（平台限制，见"已知限制"），按钮会提示改用下方逐段导航；在**电脑上**打开时点它可看高德网页版的完整多点路线（上限 16 个途经点）
...
- 高德导航 URI 只支持单目的地，所以逐段导航是"一段一次"形式；「导航全部」在 3 个点以内用官方 URI 调起 App（via 只支持 1 个途经点）。**手机上没有能一次导入多个途经点的链接方式**：网页版 `amap.com/dir` 的多途经点参数在 PC 浏览器可用（上限 16 个），但手机浏览器会被强制跳转 m.amap.com 并丢弃全部参数——所以多点的路书在手机上请用逐段导航；点名称建议写成 `D1 xxx` 便于区分
...
- 2026-09-17 「导航全部」按端分流：实测手机浏览器打开 amap.com/dir 会被强制跳 m.amap.com 并丢弃途经点参数，手机上多途经点无链接方案——手机端 >3 个点时按钮明确提示改用逐段导航，PC 端仍打开网页版完整路线
